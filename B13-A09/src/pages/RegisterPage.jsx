                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {strengthItems.map(({item }) => (
                        <div key={item.key} className={`flex items-center gap-1.5 text-xs ${strength[item.key] ? "text-success" : "text-base-content/40"}`}>
                          <FiCheckCircle className={strength[item.key] ? "fill-current" : ""} />
                          {item.label}
                        </div>
                      ))}
                    </div>
