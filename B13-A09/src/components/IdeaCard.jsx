    ...
    <div className="idea-card card bg-base-100 border border-base-300 hover:border-violet-300 hover:shadow-xl hover:shadow-violet-100/50 dark:hover:shadow-violet-900/20 rounded-2xl overflow-hidden flex flex-col h-full">
      ...
      <figure className="relative h-44 overflow-hidden bg-base-200">
        <img
          ...
          onError={(e) => {
            e.target.src = `https://placehold.co/600x400/7c3aed/ffffff?text=${encodeURIComponent(
        ...
        <div className="absolute top-3 left-3">
          <span
            className={`badge ${CATEGORY_COLORS[idea.category]} badge-sm font-semibold gap-1`}
          >
          ...
          <div className="avatar">
            <div className="w-7 h-7 rounded-full ring-1 ring-base-300">
              <img
                ...
                alt={idea.authorName}
                onError={(e) => {
                  e.target.src = `https://api.dicebear.com/7.x/initials/svg?seed=${idea.authorName}`;
                }}
              />
            </div>
          </div>
        ...
        {idea.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {idea.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="badge badge-ghost badge-xs font-medium text-violet-600 bg-violet-50 dark:bg-violet-900/20 border-violet-200"
              >
                #{tag}
              </span>
            ))}
