...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    updateProfile({ name: form.name.trim(), photoURL: form.photoURL });
    setSaving(false);
    setEditing(false);
    ...
    <div className="min-h-screen bg-base-100 pb-12">
      ...
      <div className="bg-gradient-to-br from-violet-600 to-purple-700 py-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <h1 className="text-3xl sm:text-4xl font-black text-white mb-2">Profile Settings</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10 space-y-6">
        ...
        <div className="card bg-base-100 border border-base-300 rounded-3xl shadow-xl overflow-hidden">
          ...
          <div className="p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center border-b border-base-300">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-black text-base-content">{user?.name}</h2>
          ...
          {editing && (
            <div className="p-6 sm:p-8 space-y-5">
              <h3 className="font-bold text-base text-base-content">Edit Information</h3>
              ...
              <div className="form-control">
                <label className="label font-semibold text-sm pb-1">Photo URL</label>
                <div className="relative">
                ...
                {form.photoURL && (
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    />
                    <span className="text-sm text-base-content/60">Photo preview</span>
                  </div>
                )}
              </div>
        ...
        <div className="card bg-base-100 border border-base-300 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: "Ideas Posted", value: userIdeasCount, icon: "💡", color: "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400" },
              { label: "Total Upvotes", value: totalUpvotes, icon: "🔥", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400" },
              { label: "Comments Made", value: totalComments, icon: "💬", color: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" },
              { label: "Bookmarks", value: totalBookmarks, icon: "🔖", color: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400" },
            ].map((stat) => (
              <div key={stat.label} className={`rounded-2xl p-4 ${stat.color} text-center`}>
        ...
        {/* Recent Activity */}
        <div className="card bg-base-100 border border-base-300 rounded-2xl p-6">
          <h3 className="font-bold text-lg mb-5">Recently Trending in Your Categories</h3>
          <div className="space-y-3">
            {mockIdeas.slice(0, 3).map((idea) => (
              <div key={idea._id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-base-200 transition-colors">
                <img
                  src={idea.imageURL}
                  alt={idea.title}
                ...
                <span className="badge badge-sm font-semibold text-violet-600 bg-violet-50 border-violet-200">View</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.label} className="flex items-center gap-3 p-3 rounded-xl bg-base-200">
                ...
                <div className="flex-1">
                  <div className="text-xs text-base-content/50 mb-0.5">{item.label}</div>
